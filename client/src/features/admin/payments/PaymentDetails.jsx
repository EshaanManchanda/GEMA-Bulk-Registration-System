...
const getFullUrl = (url) => {
  ...
  // Remove /api/v1 or /api from the end to get base server URL
    ...
    <AdminLayout>
      <div className="max-w-7xl mx-auto space-y-6">
            ...
            <Card.Body>
              <div className="flex items-center gap-6">
                <div className="relative w-48 h-48">
                  ...
                  />
                  <div
                    className="w-48 h-48 bg-gray-100 rounded-lg border border-gray-300 items-center justify-center flex-col gap-2 cursor-pointer hover:bg-gray-200 hidden"
                    onClick={() => setShowReceiptModal(true)}
                  >
                    <svg className="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                    <span className="text-xs text-gray-500 text-center px-2">Click to view receipt</span>
                  </div>
                </div>
                <div>
        ...
        >
          <div className="space-y-4">
            {/* Image Container */}
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-2 flex justify-center items-center overflow-auto" style={{ minHeight: '400px', maxHeight: '70vh' }}>
              <img
                src={getFullUrl(receiptUrl)}
                alt="Payment Receipt"
            ...
            <div className="flex justify-center gap-3 pt-2">
              <Button
                ...
                onClick={async () => {
                  try {
                    const response = await fetch(getFullUrl(receiptUrl));
                    const blob = await response.blob();
                    const url = window.URL.createObjectURL(blob);
                    const link = document.createElement('a');
                    link.href = url;
                    link.download = `receipt-${payment?.payment_reference || 'payment'}.${receiptUrl?.split('.').pop() || 'png'}`;
                    ...
                    window.open(getFullUrl(receiptUrl), '_blank');
                  }
                }}
              >
